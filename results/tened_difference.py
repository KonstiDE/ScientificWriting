...
if __name__ == '__main__':
    ...
    for m in range(1, 11):
        current = "B:/projects/PycharmProjects/ScientificWriting/output/best_of_models/results_single_BCEWithLogitsLoss_L1Loss_Adam_Adam_UNET_SHAPE_UNET_HEIGHT_1e-05_1e-05_below" + str(
            m)

        os.chdir(current)

        tr_loss_shapes, val_loss_shapes, tr_loss_heights, val_loss_heights, tr_mses, val_mses, \
        tr_ssims, val_ssims, tr_accs, val_accs = find_max_epoch(current)

        plt.plot(tr_loss_heights, c='blue')
    ...
    plt.plot(tr_loss_heights, c='red')
